...
#macro	__ANIMATION_POOL	global.__animation_pool
__ANIMATION_POOL		= new ListPool();
...
function Animation(_obj_owner, _delay, _duration, _animcurve, _repeats = 1) constructor {
	...
	func_image_blend	= function(value) { owner.image_blend	= value; };
	...
	func_image_angle	= function(value) { owner.image_angle	= value; };
	...
	/// @description				use this function if the animcurve holds a standard 0..1 value
	///								for x/y and the curve value shall be a multiplier to the 
	///								target coordinates you supply here.
	///								This function is pretty much the same as set_move_distance
	///								(which allow a relative movement), but this sets an absolute target.
	///								Both default move functions for x and y respect this setting
	...
	/// @function					set_scale_is_relative(relative)
	...
	static set_scale_is_relative = function(relative) {
	...
	static step = function() {
		...
		if (__active) {
			...
			if (animcurve != undefined) {
				animcurve.update_values(__frame_counter / duration);
